module Validation
  ...
  module ClassMethods
    ...
    @@error_messages = {
      presence: "should be neither nil nor an empty string",
      format: "unmatch the passed regular expression ",
      type: "not an instance of the passed class "
    }
    # @return a rule container`s name for each class
    def r_name
    ...
    # @action add new rule in validation list
    def validate name, rule
      if @@types.include?( rule.keys.first )
        rules[name] = rule 
      else
  ...
  def valid?
    self.class.rules.each do |key, val|
      return false unless validate_attribute key, val
    end 
    true   
  end
  ...
  def validate!
    self.class.rules.each do |key, val|
      validate_attribute key, val, true
    end 
    true   
  end
  ...
  def presence? name, rule
    ...
    !point.nil? && !(point.is_a?(String) || point.strip!.empty? && point.empty?)
  end
